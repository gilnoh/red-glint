...
# The script gets gets 
# integer-ID-lized feature-vector per-line format 
# and outputs feature x feature inner-product matrix. 
# (rows of so-called B matrix. See /memo to check what is a B matrix)
#
# The script gets three arguments from command line: 
# arg0: feature_vector file name (integer ID based feature vectors, output of test2.rb, from e-f-s file)
# arg1: row-from  (the start row number of B matrix) 
# arg2: row-to    (the end row number of B matrix) 
# 
# The script will generate a output file with OUTFILE_PREFIX + "from" + "to"
# as line-indexed file (File_with_row_index); and outputs rows of B 
# matrixes (one per line), starting from row-from (arg1), to row-to (arg2) 
#
# (To save disc space, the output has relative-index.) 
# output format is: 
# [row_id]\t\t[r-index]:[val]\t[r-index]:[val] ... [r-index]:[val]\n 
# r-index is relative index, where it records delta (index - previous index) 
